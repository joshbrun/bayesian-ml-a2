...
from src.clustering_occupancy.train_gmm import train
import os
...
ANALYSIS_FILE = os.path.join(ANALYSIS_PATH, "gmm_clustering.csv")
ORDERED_FEATURES = os.path.join(ANALYSIS_PATH, "gmm_features.csv")
...
def run(options):
    ...
    if training:
        ...
        if not os.path.isdir("models"):
            os.mkdir('models')
        joblib.dump(model, 'models/gmm_model.joblib')
        ...
        if not os.path.isdir("models"):
            ...
            exit(2)
        model = joblib.load('models/gmm_model.joblib')
