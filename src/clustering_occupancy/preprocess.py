...
"""
SE755 - A2
Runs preprocessing on occupancy data
...
def preprocess(raw_data, n, feature_list=None, evaluation_data=None):
    """
    preprocess the occupancy data
    :param data: the raw occupancy data
    :return: the preprocessed occupancy data
    """
    ...
    if evaluation_data is not None:
        data = preprocessed_data.iloc[17120:, :].copy()
        features, target = split_input_and_target(data)
        return features
    else:
...
def feature_extraction(data, n, feature_list):
    ...
    if feature_list is not None:
        ...
        most_correlated = data.corr().abs()['Occupancy'].sort_values(ascending=False)
        ...
        best_columns = numpy.append(best_columns, ['Occupancy'])
